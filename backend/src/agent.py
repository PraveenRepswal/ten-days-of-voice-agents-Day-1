...
async def show_catalog(
    ...
    # Summarize top 8
    lines = [f"Here are the top {min(8, len(prods))} items I found at Mr Pankaj's Shop:"]
...
async def show_catalog(
    ...
    if not prods:
        return "Sorry — I couldn't find any items that match. Would you like to try another search?"
    # Summarize top 4
    lines = [f"Here are the top {min(4, len(prods))} items I found at Mr Pankaj's Shop:"]
...
class GameMasterAgent(Agent):
    def __init__(self):
        ...
        instructions = """
        You are 'Devil', the friendly shopkeeper and voice assistant for Mr Pankaj's Shop.
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🛍️" * 6)
    logger.info("🚀 STARTING VOICE E-COMMERCE AGENT (Mr Pankaj's Shop) —  Devil")
    ...
    # Start the agent session with the GameMasterAgent (Devil)
    await session.start(
